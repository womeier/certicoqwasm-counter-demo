...
(func $counter_receive_wrapper(export "counter.inc") (param i64) (result i32)
  ...
  (; increment by 2 ;)
  (local.set $state_new (call $counter_receive (local.get $state) (i64.const 2)))
  (; return on failure ;)
...
(func $counter_receive (param $count_old i64) (param $increase_by i64) (result i64)
  ...
  (local.set $res (i32.load (local.get $res)))
;;  (local.set $res (call $get_result_monad_ok (local.get $res))) ;; get result Ok
;;  (local.set $res (call $call_closure (local.get $state_to_i63_clos) (local.get $res)))

;;  (return (call $i63_to_i64 (local.get $res)))
