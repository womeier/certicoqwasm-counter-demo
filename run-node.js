...
(async () => {
    ...
    try {
        ...
        if (obj.instance.exports.result_out_of_mem.value == 1) {
            ...
            console.log(`Benchmark: {{"time_startup": "${time_startup}", "time_main": "${time_main}"}} (in ms)`);
            process.exit(1);
        } else {
            const res_value = obj.instance.exports.result.value;
            process.stdout.write("====> ");

/*            let fn_pp = pp_map[program];
            if (fn_pp == undefined) {
            ...
            const tag = dataView.getInt32(res_value+8, true);
            console.log(tag);

//            fn_pp(res_value, dataView);
        }
//        console.log(`\nBenchmark: {{"time_startup": "${time_startup}", "time_main": "${time_main}", "time_pp": "${time_pp}", "bytes_used": "${bytes}", "program": "${program}"}} ms, bytes.`);
    } catch (error) {
        console.log(error);
        process.exit(1);
    }
